<template>
    <div class="card">
        <DataTable v-model:filters="filters" v-model:selection="selectedData" :value="filteredData" paginator :rows="10"
            ...
            <!-- 评分 (仅 volunteer) -->
            <Column v-if="props.role === 'volunteer'" field="rating" header="Rating" style="min-width: 8rem" sortable>
            </Column>
            <Column header="Send Email" style="min-width: 10rem">
                <template #body="{ data }">
                    <Button label="Send Email" @click="sendEmail(data)" class="p-button-sm p-button-success" />
                </template>
            </Column>
        </DataTable>
        ...
        <div class="mt-3">
            ...
            <Button label="Send Email to Selected" class="ml-2 p-button-warning" @click="sendEmailsToSelected"
...
const sendEmail = async (user) => {
    ...
    try {
        ...
        console.log("Error sending email front end", error)
    }
}

async function sendEmailsToSelected() {
    try {
        const emails = selectedData.value.map(user => ({
            email: user.email,
            name: user.name
        }));
...
// 导出 CSV 功能
function exportCSV() {
    const csvData = selectedData.value.map(row =>
        props.role === 'user'
            ? `${row.name},${row.gender},${row.email}`
            : `${row.name},${row.gender},${row.email},${row.rating}`
    ).join('\n');

    const blob = new Blob([props.role === 'user'
        ? `Name,Gender,Email\n${csvData}`
